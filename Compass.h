...
class Compass {
public:
    Compass() : update_sensor_1(false), reportType(SH2_ARVR_STABILIZED_RV), reportIntervalUs(100000L) {}
    float getHeading(Adafruit_BNO08x* bno08x, int degree_shift);
    long getReportInterval() const { return reportIntervalUs; }
    static void setReports(Adafruit_BNO08x* bno08x, sh2_SensorId_t reportType, long report_interval);
